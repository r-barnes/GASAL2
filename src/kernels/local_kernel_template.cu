...
template<Bool B>
__device__ void CORE_LOCAL_COMPUTE(
...
__device__ void compute_local_block(
  ...
  for(int32_t l = 28, m = 1; m < 9; l -= 4, m++) {
    ...
    CORE_LOCAL_COMPUTE<B>(gbase, rbase, p[m], e, h[m], f[m]);
...
#define CORE_LOCAL_COMPUTE_START() \
    ...
    subScore = DEV_GET_SUB_SCORE_LOCAL(rbase, gbase); \
  ...
  for (int i = 0; i < target_batch_regs; i++) { //target_batch sequence in rows
    ...
    for(int j = 0; j < query_batch_regs; j+=1) { //query_batch sequence in columns
      ...
      if (S==CompStart::WITH_TB) {
        ...
        uint32_t rbase = (rpac >> 28) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        auto e = HD.y;
        int32_t l, m;
        ...
        rbase = (rpac >> 24) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        rbase = (rpac >> 20) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        rbase = (rpac >> 16) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        rbase = (rpac >> 12) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        rbase = (rpac >> 8) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        rbase = (rpac >> 4) & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        rbase = rpac & 0xF;//get a base from query_batch sequence
        HD = global[ridx];
        h[0] = HD.x;
        e = HD.y;
        for (l = 28, m = 1; m < 9; l -= 4, m++) {
        ...
        for (int32_t k = 28; k >= 0; k -= 4) {
          ...
          compute_local_block<B>(gpac, rbase, gidx, e, maxXY_y, maxHH, h, f, p, maxHH_second, maxXY_y_second);
